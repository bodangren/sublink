...
const InvoiceForm = ({ editId, initialData }: InvoiceFormProps) => {
  ...
  const [lineItems, setLineItems] = useState<InvoiceLineItem[]>(initialData?.lineItems || [createEmptyLineItem()])
  ...
  useEffect(() => {
    if (projectId) {
      getTimeEntriesByProject(projectId).then(setAvailableTimeEntries)
  ...
  const addTimeEntryAsLineItem = (entry: TimeEntry) => {
    ...
    const newItem: InvoiceLineItem = {
      id: crypto.randomUUID(),
      type: 'time',
      description: entry.notes || `Work on ${new Date(entry.startTime).toLocaleDateString()}`,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!issueDate || !dueDate) {
      setError('Please enter issue and due dates')
      return
    }
  ...
  return (
    <div className="container">
      <h1>{editId ? 'Edit Invoice' : 'Create Invoice'}</h1>

      <form aria-label="invoice form" onSubmit={handleSubmit} style={{ marginTop: '1.5rem' }}>
        {error && (
          <div style={{
        ...
        <div style={{ marginBottom: '1.5rem' }}>
          ...
          {availableTimeEntries.length > 0 && (
            <div style={{ marginBottom: '1rem', padding: '0.75rem', backgroundColor: 'var(--secondary-bg)', borderRadius: '4px' }}>
              <p style={{ fontWeight: 'bold', marginBottom: '0.5rem' }}>Add Time Entries:</p>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                {availableTimeEntries.slice(0, 5).map(entry => (
                  <button
                    key={entry.id}
                    type="button"
                  ...
                  >
                    + {Math.round(entry.duration / 3600 * 100) / 100}h ({new Date(entry.startTime).toLocaleDateString()})
                  </button>
                ))}
              </div>
            </div>
          ...
          {lineItems.map((item, index) => (
            <div key={item.id} style={{ 
              ...
              borderRadius: '4px',
              border: item.type === 'time' ? '2px solid var(--accent-color)' : '1px solid var(--border-color)',
            }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                <span style={{ fontWeight: 'bold' }}>Item {index + 1} {item.type === 'time' && '(Time Entry)'}</span>
                {lineItems.length > 1 && (
                  <button
                    type="button"
