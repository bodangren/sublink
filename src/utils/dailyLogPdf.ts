...
export const generateDailyLogPdf = async ({ log, photos }: GenerateDailyLogPdfOptions): Promise<void> => {
  ...
  doc.text('Daily Construction Log', PAGE_MARGIN, yPosition)
  yPosition += 15
  ...
  doc.text(projectLines, PAGE_MARGIN, yPosition)
  yPosition += projectLines.length * 7 + 5
  
  doc.setFontSize(11)
  doc.setTextColor(80, 80, 80)
  doc.text(formatDate(log.date), PAGE_MARGIN, yPosition)
  
  if (photos.length > 0) {
    doc.text(`${photos.length} photo${photos.length !== 1 ? 's' : ''}`, pageWidth - PAGE_MARGIN - 30, yPosition)
  }
  yPosition += 15
  ...
  const addSection = (title: string, content: string, borderColor?: [number, number, number]) => {
    ...
    if (borderColor) {
      ...
      doc.line(PAGE_MARGIN, yPosition, PAGE_MARGIN, yPosition + 3)
    }
    
    doc.setFontSize(11)
    doc.setTextColor(60, 60, 60)
    doc.text(title, PAGE_MARGIN + (borderColor ? 3 : 0), yPosition)
    yPosition += 6
    
    doc.setFontSize(10)
    doc.setTextColor(0, 0, 0)
    const lines = doc.splitTextToSize(content, contentWidth - (borderColor ? 3 : 0))
    doc.text(lines, PAGE_MARGIN + (borderColor ? 3 : 0), yPosition)
  ...
  if (photos.length > 0) {
    ...
    doc.setFontSize(14)
    doc.setTextColor(0, 0, 0)
    doc.text('Site Photos', PAGE_MARGIN, yPosition)
    yPosition += 10
    ...
    for (let i = 0; i < photos.length; i++) {
      ...
      const photoX = PAGE_MARGIN + col * (photoWidth + photoSpacing)
  ...
  doc.setFontSize(9)
  doc.setTextColor(100, 100, 100)
  doc.text(`Generated: ${new Date().toLocaleString()}`, PAGE_MARGIN, yPosition)
  ...
  const dateStr = formatShortDateFromStr(log.date)
